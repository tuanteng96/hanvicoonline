...
export default class extends React.Component {
    ...
    getDiary = () => {
        ...
        UserService.getDiary(member, password)
            .then(response => {
    ...
    render() {
        ...
        return (
            <Page ptr onPtrRefresh={this.loadMore.bind(this)} name="diary" className="diary">
                <Navbar>
                ...
                <div className="diary-box">
                    <div className="diary-box__list">
                        {
                            arrDiary && arrDiary.length > 0 ? arrDiary.map((item, index) => (
                                <div className="diary-box__list-item" key={index}>
                                    <h4><span>{this.getStuff(item.dayFull)}</span><span>{this.getDate(item.dayFull)}</span></h4>
                                    <ul>
                                        {
                                            item.items.map((sub, i) => (
                                                <li key={i}>
                                                    <div className="icon"><span></span></div>
                                                    <div className="time">{this.getTime(sub.CreateDate)}</div>
                                                    <div className="text">
                                                        {ReactHtmlParser(sub.Content)}
                                                    </div>
                                                </li>
                                            ))
                                        }
